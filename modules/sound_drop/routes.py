"""Blueprint Flask del módulo sound_drop."""
import threading
...
from modules.sound_drop import processor
from modules.sound_drop.schema import SoundDropParams

bp = Blueprint("sound_drop", __name__, url_prefix="/api/sound-drop")
...
def process():
    ...
    if not audio.filename:
        return jsonify(error="Nombre de audio vacío"), 400
    ...
    try:
        params = SoundDropParams.from_form(request.form)
    ...
    thread = threading.Thread(
        ...
        args=(job_id, video_path, audio_path, output_path, params),
        daemon=True,
    )
    thread.start()
...
def download(job_id):
    ...
    return send_file(
        ...
        mimetype="video/mp4",
        as_attachment=True,
        download_name=f"reelforge_sound_{job_id}.mp4",
    )
